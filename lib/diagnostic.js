...
const min = function (workingArray, callback) {
  const newArray = [];
  workingArray.reduce( function(element, index, array) {
    if (callback(element, index, array)) {
      newArray.push(element);
    }
  });
  let minimum = array.reduce(newArray);
  return minimum;
};
...
const sum = function (workingArray, callback) {
  workingArray.sum(function (a, b) {
    return a + b;
  };
};
