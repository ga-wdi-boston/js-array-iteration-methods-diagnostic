...
const min = function (previous, current) {
  return previous.reduce(previous, (a, e, i, array) => a < e ? a : e)
}
arr.map(function(number) {return (number + 1)})
// Question 2
...
const sum = function (previous, current) {
  return previous.reduce(previous, (a, e, i, array) => previous + current)
}
...
const hasOne = function (array, callback) {
  // if callack = true for only one
  // --> filter by callback & count results
  if (array.filter(callback(array[i], i, array)).length === 1) {
    return true
  } else {
    return false
  }
