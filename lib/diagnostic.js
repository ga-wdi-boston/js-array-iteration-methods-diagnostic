...
const min = function () {                       //object min is a function (which is the minimum in the array)
  let reduce = function (array, callback) {       //variable reduce is a function which reduces the array
    for (i = 0, i < array.length; i++) {  //loops through the array
    callback(reduce, e, array[i], a);    //uses callback to reduce to minimum #
  });
}
  return array.min;
};
