...
const min = arr.reduce(previous, current) {
  let minimum = array.reduce(min) => current < previous ? current : previous
}
...
const hasNone = arr.filter(array, callback) {
  return
}
...
// you may use any feature of JavaScript
// to solve this problem
const hasOne = arr.filter((array, callback)) {
  return true
}
