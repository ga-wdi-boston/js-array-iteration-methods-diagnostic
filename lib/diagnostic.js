...
function arrayMin(arr) {
  const len = arr.length, min = Infinity;
  while (len--) {
    if (arr[len] < min) {
      min = arr[len];
    }
...
***I know this is far from from the correct answer, but it is an example of setting a condition
that needs to be adhered, so (by my probably faulty logic) similar to a
