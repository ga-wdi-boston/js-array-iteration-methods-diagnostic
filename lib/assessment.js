'use strict';

// All of the callbacks in questions 3 through 5 should
// be called with the current element in the array,
// its index, and the array itself
// e.g. callback(currentValue, index, array)

Matt or Saad: this has been a rough day for assessments, I will do whatever it takes to learn this stuff I just need some help.  I feel like I need to start over on the lesson for call backs then move into the four potential array methods.

// Question 1
// Write a callback for reduce that selects
// the minimum value in the array

var min = function min() {
 [0, 1, 2, 3, 4].reduce(function(previousValue, currentValue, index, array)
  if ([0]<){
  return [i];
} else {
  return +=i;
}
});


// Question 2
// Write a callback for reduce that produces
// the sum of the values in the array
var num = [1,3,5,7,9]

var sum = function sum() {
  num.reduce(sum(previousValue, currentValue, index, array){
return previousValue + currentValue
  }
};

// Question 3
// Write a function that returns true
// if invoking the callback for any element
// in the array returns a truthy value
var any = function any(array, callback, thisArg) {

};

// Question 4
// Write a function that returns a new array
// made up of the array elements for which invoking
// the callback returns a truthy value
var select = function select(array, callback, thisArg) {

};

// Question 5
// Write a function that returns the first element
// for which invoking the callback returns a truthy value
var find = function find(array, callback, thisArg) {

};

// Used by testing
module.exports = {
  min: min,
  sum: sum,
  any: any,
  select: select,
  find: find
};
